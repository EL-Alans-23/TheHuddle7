"""Capa de datos del tasks-service (Database per Service).

Define el modelo `Tarea`, el engine de SQLAlchemy hacia su PostgreSQL
INDEPENDIENTE y la rutina de inicialización que espera a que la base de datos
esté lista antes de crear las tablas.

Nota de arquitectura: `user_id` referencia a un usuario que vive en OTRO
servicio (users-service). Por eso NO se usa una ForeignKey real entre bases de
datos distintas; la integridad se valida vía API / JWT, no con un constraint
de PostgreSQL.
"""
...
class Base(DeclarativeBase):
    """Base declarativa exclusiva del esquema de tareas."""


class Tarea(Base):
    """Tabla 'tareas': trabajo a realizar y su estado."""
    ...
    # ID lógico del usuario propietario (vive en users-service, sin FK cruzada).
